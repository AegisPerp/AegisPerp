...
// Tradable perp markets ($ALPERP itself is NOT tradable — it's the brand index).
...
export function FeedProvider({ children }: { children: ReactNode }) {
  ...
  const [alperp, setAlperp] = useState<IndexStat>({ price: 24.8, change24h: 0 });
  const [connected, setConnected] = useState(false);
  const [source, setSource] = useState("connecting…");
  const [selected, setSelected] = useState("SOL");
  const ref = useRef<Market[]>(markets);
  ref.current = markets;
  const alpRef = useRef(alperp);
  alpRef.current = alperp;
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      if (!changed) return;
      setMarkets(next);
      // $ALPERP brand index = average of the live markets' 24h change
      ...
      setAlperp({ price: +(a.price * (1 + (n ? driftSum / n : 0))).toFixed(4), change24h: +chg.toFixed(2) });
    }, 1000);
  ...
  const value: FeedValue = {
    markets, alperp, connected, source, selected, setSelected,
